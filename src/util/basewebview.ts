...
export class AvaWebView {
    ...
    public static getOrCreateRequestViewPanel = (title: string): vscode.WebviewPanel | undefined => {
        try {
            ...
            if (!panel) {
                ...
                if (panelTitle.split(' ')[0].toLowerCase() === title.split(' ')[0].toLowerCase() && panelTitle.split(' ')[1].toLowerCase() === title.split(' ')[1].toLowerCase()) {
                    if (!panel.visible) {
    ...
    public static createModelViewPanel(title: string): vscode.WebviewPanel {
        ...
        const columnToShowIn = vscode.window.activeTextEditor ? vscode.window.activeTextEditor.viewColumn || vscode.ViewColumn.One : vscode.ViewColumn.One;
