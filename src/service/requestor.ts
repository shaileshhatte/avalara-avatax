...
import { getConfiguration } from '../util/configurationProvider';
...
const ACCOUNT_NUMBER: string = config[`avataxaccountnumber`]; // '2000928076';
const LICENSE_KEY: string = config[`avataxlicensekey`]; //'64E43E6D81DA7113';
const REQUEST_TIMEOUT: number = config[`requesttimeout`]; // 0
const ENV_TYPE: string = config[`environment`];
const BASE_URL: string = ENV_TYPE === 'Sandbox' ? `https://sandbox-rest.avatax.com` : 'https://rest.avatax.com';
...
export const fireRequest = (data: any): any => {
	if (!ACCOUNT_NUMBER || !LICENSE_KEY) {
		vscode.window.showErrorMessage(
			`AvaTax account credentials missing. Go to Settings > Extensions > Avalara, and configure your AvaTax credentials.`
		);
		return;
	}
	setup(data);
